...
export class Dap2SourceList extends AbstractSourceList {
   ...
   */
  constructor() {
    super(sourceListConfig[SourceList.DAP2]);
  }
  ...
  protected async prepare(data: DataFrame): Promise<DataFrame> {
    // Only keep records that have 100 or more pageviews
    //@ts-ignore
    data = data.filter((row: any) => row.get("pageviews") >= 100);

    // Drop Unnecessary Columns
    data = data.drop("pageviews");
    data = data.drop("visits");

    // Rename Columns for Standardization
    data = data.rename("hostname", "target_url");
