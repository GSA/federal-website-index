...
export class UsaGovClicksMilSourceList extends AbstractSourceList {
  /**
   * Provides the entry point for loading data from this source list.
   */
  static async loadData(): Promise<DataFrame> {
    const loader = new UsaGovClicksMilSourceList();
    return loader.load();
  }
   ...
   */
  constructor() {
    super(sourceListConfig[SourceList.USAGOV_CLICKS_MIL]);
  }
