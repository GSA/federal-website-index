...
export class HyperlinkDomainsSourceList extends AbstractSourceList {
  /**
   * Provides the entry point for loading data from this source list.
   */
  static async loadData(): Promise<DataFrame> {
    const loader = new HyperlinkDomainsSourceList();
    return loader.load();
  }
   ...
   */
  constructor() {
    super(sourceListConfig[SourceList.HYPERLINK_DOMAINS]);
  }
  ...
  protected async prepare(data: DataFrame): Promise<DataFrame> {
    ...
    data = data.rename("domains", "target_url");
