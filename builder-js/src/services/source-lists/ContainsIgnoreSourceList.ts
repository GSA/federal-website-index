...
export class ContainsIgnoreSourceList extends AbstractSourceList {
  /**
   * Provides the entry point for loading data from this source list.
   */
  static async loadData(): Promise<DataFrame> {
    const loader = new ContainsIgnoreSourceList();
    return loader.load();
  }
   ...
   */
  constructor() {
    super(sourceListConfig[SourceList.CONTAINS_IGNORE]);
  }
  ...
  protected async prepare(data: DataFrame): Promise<DataFrame> {
    ...
    data = data.rename('URL contains between non-word characters:', 'contains_string');
